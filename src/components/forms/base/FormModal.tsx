...
const VideoLessonForm = dynamic(() => import("../VideoLessonForm"));
...
const FormModal = ({
  ...
  const sizeClass = type === "create" ? styles.sm : styles.xs;
  ...
  const Form = () => {
    ...
    useEffect(() => {
      if (state.success) {
        toast(`${table} deletado com sucesso!`);
        setOpen(false);
        router.refresh();
      }
    }, [state]);
    ...
    if (type === "delete" && id) {
      return (
        <form action={formAction} className={styles.deleteWrapper}>
          <input type="hidden" name="id" value={id} />
          <p className={styles.deleteText}>
            Todos os dados serão perdidos. Deseja realmente deletar este {table}?
          </p>
